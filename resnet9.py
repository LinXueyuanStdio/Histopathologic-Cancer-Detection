...
TRAIN_IMAGES_FOLDER = train_path
...
def auc_writer(y_true, y_hat, iteration):
    try:
        ...
        auc = -1
    logger.info('iteration: {}, auc: {}'.format(iteration, auc))
...
# implot(files)
#
# files = predictions[predictions['true'] == 0].sort_values('pred', ascending=False)['files'].values[
#         :max_charts]
# implot(files)
#
# files = predictions[predictions['true'] == 1].sort_values('pred', ascending=False)['files'].values[
#         :max_charts]
# implot(files)
#
# files = predictions[predictions['true'] == 0].sort_values('pred', ascending=True)['files'].values[
#         :max_charts]
# implot(files)
